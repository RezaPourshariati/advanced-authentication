...
import { generateRefreshToken, generateToken, hashToken } from '../utils/index.js'
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!allowedAgent) {
    ...
    const userToken = await Token.findOne({ userId: user._id }) // Token is from tokenModel.js file
    if (userToken)
...
const sendVerificationEmail = asyncHandler(async (req, res) => {
  ...
  const token = await Token.findOne({ userId: user._id }) // Token is from tokenModel.js file
  if (token)
